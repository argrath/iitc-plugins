...
window.plugin.differentialScore = function() {};

// ensure from-iitc-core framework
if(typeof window.iitccore !== 'function') window.iitccore = function() {};

var setup = function() {
  window.addHook('iitcLoaded', function() {
    window.regionScoreboard = window.iitccore.regionScoreboard;
    window.iitccore.regionScoreboardScoreHistoryChart = window.plugin.differentialScore.chart;
  });
}

window.plugin.differentialScore.chart = function (result, logscale) {
...
window.iitccore.regionScoreboard = function() {
  ...
  window.postAjax('getRegionScoreDetails', {latE6:latE6,lngE6:lngE6}, function(res){window.iitccore.regionScoreboardSuccess(res,dlg);}, function(){regionScoreboardFailure(dlg);});
}

window.iitccore.regionScoreboardScoreHistoryTable = function (result) {
...
window.iitccore.regionScoreboardSuccess = function (data,dlg,logscale) {
  ...
  dlg.html('<div class="cellscore">'
         ...
         +window.iitccore.regionScoreboardScoreHistoryChart(data.result, logscale)+'</div>'
         +'<b>Checkpoint overview</b>'
         +'<div>'+window.iitccore.regionScoreboardScoreHistoryTable(data.result)+'</div>'
         +'<b>Top agents</b>'
         +'<div>'+agentTable+'</div>'
         +'</div>');
  ...
  $('input.logscale', dlg).change(function(){
    var input = $(this);
    window.iitccore.regionScoreboardSuccess(data, dlg, input.prop('checked'));
  });
}
